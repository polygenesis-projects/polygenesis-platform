...
package com.arcaiv.contexts.auth.domainservice;
  ...
  public static Thing create(Thing user, PackageName rootPackageName) {
    Thing userDomainService =
        ThingBuilder.domainService("userDomainService")
            .setPreferredPackage(rootPackageName.withSubPackage("user").getText())
            .createThing(rootPackageName);

    userDomainService.addFunction(
        FunctionBuilder.of(
            userDomainService,
            "assertAvailabilityOfEmail",
            "",
    ...
    userDomainService.addFunction(
        FunctionBuilder.of(
            userDomainService,
            "ensureExistenceOfUserWithCredentials",
            "",
            Purpose.genericCommand(),
            FunctionRole.userAsSet())
            .addArgument(DataPrimitive.of(PrimitiveType.STRING, new VariableName("email")))
