...
public class Tenant {
  ...
  public static Thing create(String rootPackageName, Thing user) {
    Thing tenant = ThingBuilder.endToEnd("tenant").createThing();
  ...
  private static Set<Data> createData(String rootPackageName, Thing user) {
    return DataBuilder.create()
        .withReferenceToThingById(rootPackageName, user, "ownerUserId")
        .withTextPropertyToValueObject(
            "name",
            Shared.name(rootPackageName).getObjectName(),
            Shared.name(rootPackageName).getPackageName())
        .build()
        .build();
  }
}
