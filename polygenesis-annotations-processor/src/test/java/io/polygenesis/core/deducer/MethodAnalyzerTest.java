...
public class MethodAnalyzerTest {
  ...
  public void shouldGetMethodInput() {
    ...
    BDDMockito.given(parameter.getType()).willAnswer((Answer<Class<?>>) invocation -> Object.class);
    BDDMockito.given(parameter.getName()).willReturn("some parameter name");
  ...
  public void shouldFailForInvalidClass() {
    MethodAnalyzer methodAnalyzer = new MethodAnalyzer();
    Method someMethod = Mockito.mock(Method.class);
    ...
    BDDMockito.given(someMethod.getGenericReturnType()).willReturn(type);
    BDDMockito.given(type.getActualTypeArguments()).willReturn(argTypes);
    BDDMockito.given(someMethod.getReturnType())
        .willAnswer((Answer<Class<?>>) invocation -> NoArgConstructorClass.class);

    Assertions.assertThatThrownBy(() -> methodAnalyzer.getMethodOutput(someMethod))
